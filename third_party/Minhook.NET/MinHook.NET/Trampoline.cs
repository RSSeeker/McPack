    ...
    internal sealed class Trampoline{
        ...
        public Trampoline(IntPtr target, IntPtr detour, MemorySlot memorySlot ) {
            ...
            do {
                ...
                if(oldPos >= Marshal.SizeOf(typeof(JMPCALL_REL))) {
                    ...
                    if (is64) {
                        ...
                        // MODIFIED (PHASE11-AOT): SizeOfRelay (SizeOf<T>() 泛型, 无 RequiresDynamicCode)
                        ((OperandSupport)jmp).SetOperand((uint)((uint)oldInst - ((uint)newInst + SizeOfRelay(jmp))));
                    }

                    srcObj = jmp;
                    copySize = (uint)SizeOfRelay(jmp);
                    finished = true;
                    ...
                    if (is64) {
                        ...
                        ((OperandSupport)call).SetOperand((uint)dest - (uint)(newInst + SizeOfRelay(call)));
                    }

                    srcObj = call;
                    copySize = (uint)SizeOfRelay(call);
                    ...
                    if((long)Target <= (long)dest && (long)dest < ( (long)Target + Marshal.SizeOf(typeof(JMPCALL_REL)))) {
                        ...
                        if (is64) {
                            ...
                            ((OperandSupport)jmp).SetOperand((uint)((uint)dest - ((uint)newInst + SizeOfRelay(jmp))));
                        }
                    ...
                    if((long)Target <= (long)dest && (long)dest < ((long)Target + Marshal.SizeOf(typeof(JMPCALL_REL)))){
                        ...
                        if (is64) {
                            ...
                            ((OperandSupport)jcc).SetOperand((uint)((uint)dest - ((uint)newInst + SizeOfRelay(jcc))));
                        }

                        srcObj = jcc;
                        copySize = (uint)SizeOfRelay(jcc);
                    }
                ...
                if (srcObj != null) { 
                    ...
                    // MODIFIED (PHASE11-AOT): StructureToPtr(Object,..) 是 RequiresDynamicCode 重载
                    // (IL3050) —— 改用类型分派的泛型重载(全部 blittable, AOT 安全)。
                    WriteStructToPtr(srcObj, data);
            ...
            if (is64) {
                ...
                IntPtr data = Marshal.AllocHGlobal((int)SizeOfRelay(jmp));
                WriteStructToPtr(jmp, data);
                CopyMemory(Relay, data, SizeOfRelay(jmp));
                Marshal.FreeHGlobal(data);              
            }
        ...
        bool IsExecutableAddress(IntPtr address) {
            ...
            // MODIFIED (PHASE11-AOT): SizeOf<T>() 泛型 (实例重载 IL3050)
            VirtualQuery(address, ref mi, Marshal.SizeOf<MEMORY_BASIC_INFORMATION>());
            return (mi.State == MemoryState.Commited && (mi.Protect & PAGE_EXECUTE_FLAGS) > 0);
        }

        /// <summary>MODIFIED (PHASE11-AOT): blittable relay 结构 -> 原生内存 (类型分派, 替代
        /// RequiresDynamicCode 的 StructureToPtr(Object,..) 实例重载; 全部 Sequential/Pack=1)。</summary>
        static void WriteStructToPtr(object src, IntPtr data) {
            switch (src) {
                case JMPCALL_REL v: Marshal.StructureToPtr(v, data, false); break;
                case JMP_ABS v:     Marshal.StructureToPtr(v, data, false); break;
                case CALL_ABS v:    Marshal.StructureToPtr(v, data, false); break;
                case JCC_REL v:     Marshal.StructureToPtr(v, data, false); break;
                case JCC_ABS v:     Marshal.StructureToPtr(v, data, false); break;
                default: throw new ArgumentException($"unsupported relay struct: {src.GetType()}");
            }
        }

        /// <summary>MODIFIED (PHASE11-AOT): relay 结构大小 (SizeOf&lt;T&gt;() 泛型, 无 RequiresDynamicCode)。</summary>
        static int SizeOfRelay(object src) {
            switch (src) {
                case JMPCALL_REL: return Marshal.SizeOf<JMPCALL_REL>();
                case JMP_ABS:     return Marshal.SizeOf<JMP_ABS>();
                case CALL_ABS:    return Marshal.SizeOf<CALL_ABS>();
                case JCC_REL:     return Marshal.SizeOf<JCC_REL>();
                case JCC_ABS:     return Marshal.SizeOf<JCC_ABS>();
                default: throw new ArgumentException($"unsupported relay struct: {src.GetType()}");
            }
        }
